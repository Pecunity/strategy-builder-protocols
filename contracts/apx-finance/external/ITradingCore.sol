...
interface ITradingCore {
    ...
    function getPairQty(
        address pairBase
    ) external view returns (PairQty memory);
    ...
    function lastLongAccFundingFeePerShare(
        address pairBase
    ) external view returns (int256);

    function lpUnrealizedPnlUsd()
        external
        view
        returns (int256 totalUsd, LpMarginTokenUnPnl[] memory);

    function lpUnrealizedPnlUsd(
        address targetToken
    ) external view returns (int256 totalUsd, int256 tokenUsd);
