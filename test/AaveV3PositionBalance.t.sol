...
import {AaveV3PositionBalance} from "../contracts/aave-v3/condition/AaveV3PositionBalance.sol";
import {IAaveV3PositionBalance} from "../contracts/aave-v3/condition/interfaces/IAaveV3PositionBalance.sol";
...
contract AaveV3PositionBalanceTest is Test {
    ...
    function test_addCondition_Success(
        uint256 balance,
        uint8 _comparison
    ) external {
        uint8 comparison = uint8(bound((_comparison), uint8(0), uint8(5)));

        IAaveV3PositionBalance.Condition
            memory conditionToAdd = IAaveV3PositionBalance.Condition({
                asset: token,
        ...
        IAaveV3PositionBalance.Condition memory conditionAdded = condition
            .walletCondition(WALLET, conditionId);
        assertEq(conditionAdded.asset, conditionToAdd.asset);
        assertEq(
            uint8(conditionAdded.positionType),
            uint8(conditionToAdd.positionType)
        );
        assertEq(conditionAdded.balance, conditionToAdd.balance);
        assertEq(
            uint8(conditionAdded.comparison),
            uint8(conditionToAdd.comparison)
        );
    ...
    function test_isUptateable_ReturnCorrectValue(bool updateable) public {
        IAaveV3PositionBalance.Condition
            memory conditionData = IAaveV3PositionBalance.Condition({
                asset: token,
                positionType: IAaveV3PositionBalance.PositionType.COLLATERAL,
                balance: 1000,
    ...
    function test_checkCondition_COLLATERAL_Return_True(
        uint256 _balance,
        uint8 _comparison
    ) external {
        uint8 comparison = uint8(bound(_comparison, uint8(0), uint8(5)));
        ...
        vm.mockCall(
            pool,
            abi.encodeWithSelector(IPool.getReserveData.selector, token),
            abi.encode(reserveData)
        );
        vm.mockCall(
            aToken,
            abi.encodeWithSelector(IERC20.balanceOf.selector, WALLET),
            abi.encode(mockBalance)
        );
        uint8 result = condition.checkCondition(WALLET, conditionId);
        assertEq(result, 1);
    }

    function test_checkCondition_VARIABLE_DEBT_Return_True(
        uint256 _balance,
        uint8 _comparison
    ) external {
        uint8 comparison = uint8(bound(_comparison, uint8(0), uint8(5)));
    ...
    function test_checkCondition_STABLE_DEBT_Return_True(
        uint256 _balance,
        uint8 _comparison
    ) external {
        uint8 comparison = uint8(bound(_comparison, uint8(0), uint8(5)));
