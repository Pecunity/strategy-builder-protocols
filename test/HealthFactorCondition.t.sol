...
import {HealthFactorCondition} from "../contracts/aave-v3/condition/HealthFactorCondition.sol";
import {IHealthFactorCondition} from "../contracts/aave-v3/condition/interfaces/IHealthFactorCondition.sol";
...
contract HealthFactorConditionTest is Test {
    ...
    function test_addCondition_GreaterThan_Success(
        uint256 healthFactor
    ) public {
        vm.assume(healthFactor > 1e18);

        IHealthFactorCondition.Condition
            memory condition = IHealthFactorCondition.Condition({
    ...
    function test_checkCondition_Return_True(
        uint256 _healthFactor,
        uint8 _comparison
    ) public {
        uint256 healthFactor = bound(
            _healthFactor,
            1e18,
            type(uint256).max - 1
        );
        ...
        vm.mockCall(
            pool,
            abi.encodeCall(IPool.getUserAccountData, (wallet)),
            abi.encode(0, 0, 0, 0, 0, mockHF)
        );

        uint8 result = healthFactorCondition.checkCondition(
            wallet,
            conditionId
        );
        assertEq(result, 1);
