...
import {AaveV3Actions} from "../contracts/action/AaveV3Actions.sol";
...
contract AaveV3ActionsTest is Test {
    ...
    function test_supply_Success() external {
        ...
        IAction.PluginExecution[] memory executions = aaveActions.supply(WALLET, asset, amountIn);

        execute(executions);

        address aTokenAddress = IPool(AAVE_V3_POOL).getReserveData(asset).aTokenAddress;
    ...
    function test_supplyETH_Success() external {
        ...
        IAction.PluginExecution[] memory executions = aaveActions.supplyETH(WALLET, amountIn);

        execute(executions);

        address aTokenAddress = IPool(AAVE_V3_POOL).getReserveData(WETH).aTokenAddress;
    ...
    function test_withdraw_Success() external {
        ...
        IAction.PluginExecution[] memory executions = aaveActions.withdraw(WALLET, asset, amountToWithdraw);
    ...
    function test_withdrawETH_Success() external {
        ...
        uint256 amountToWithdraw = 1 ether;
        IAction.PluginExecution[] memory executions = aaveActions.withdrawETH(WALLET, amountToWithdraw);
    ...
    function test_borrow_Success() external {
        ...
        IAction.PluginExecution[] memory executions = aaveActions.borrow(WALLET, assetToBorrow, borrowAmount, 2);
        execute(executions);

        //Assert
        (,, uint256 availableBorrowsBaseAfter,,,) = IPool(AAVE_V3_POOL).getUserAccountData(WALLET);
    ...
    function test_borrowETH_Success() external {
        ...
        IAction.PluginExecution[] memory executions = aaveActions.borrowETH(WALLET, borrowAmount, 2);
        execute(executions);
    ...
    function test_repay_Success() external {
        ...
        IAction.PluginExecution[] memory executions = aaveActions.repay(WALLET, assetToBorrow, repayAmount, 2);
        execute(executions);

        assertEq(IERC20(assetToBorrow).balanceOf(WALLET), maxBorrowAmount - repayAmount);
    }
    ...
    function test_repayETH_Success() external {
        ...
        IAction.PluginExecution[] memory executions = aaveActions.repayETH(WALLET, repayAmount, 2);
        execute(executions);

        assertEq(WALLET.balance, maxBorrowAmount - repayAmount);
    }

    function test_supplyPercentageOfBalance_Success(uint256 _percentage) external {
        // Supply tokens
        ...
        IAction.PluginExecution[] memory executions = aaveActions.supplyPercentageOfBalance(WALLET, asset, percentage);
        execute(executions);

        uint256 expSupplyAmount = (percentage * maxAmount) / aaveActions.PERCENTAGE_FACTOR();
    ...
    function test_supplyPercentageOfBalanceETH_Success(uint256 _percentage) external {
        // Supply tokens
        uint256 percentage = bound(_percentage, 1, aaveActions.PERCENTAGE_FACTOR());
        ...
        IAction.PluginExecution[] memory executions = aaveActions.supplyPercentageOfBalanceETH(WALLET, percentage);
        execute(executions);
    ...
    function test_changeSupplyToHealthFactor_Success(uint256 _targetHealthFactor) external {
        ...
        IAction.PluginExecution[] memory executions =
            aaveActions.changeSupplyToHealthFactor(WALLET, asset, targetHealthFactor);
        execute(executions);
    ...
    function test_changeSupplyToHealthFactor_InvalidHealthFactor(uint256 targetHealthFactor) external {
        vm.assume(targetHealthFactor < 1e18);

        vm.expectRevert(IAaveV3Actions.HealthFactorNotValid.selector);
        IAction.PluginExecution[] memory executions =
            aaveActions.changeSupplyToHealthFactor(WALLET, WETH, targetHealthFactor);
        execute(executions);
    }

    function test_changeSupplyToHealthFactorETH_Success(uint256 _targetHealthFactor) external {
        uint256 targetHealthFactor = bound(_targetHealthFactor, 1.01e18, 10e18);
        ...
        IAction.PluginExecution[] memory executions =
            aaveActions.changeSupplyToHealthFactorETH(WALLET, targetHealthFactor);
        execute(executions);
    ...
    function test_borrowPercentageOfAvailable_Success() external {
        ...
        (,, uint256 currentAvailableBorrowsBase,,,) = IPool(AAVE_V3_POOL).getUserAccountData(WALLET);

        uint256 expBorrowBase =
    ...
    function test_borrowPerrcentageOfAvailableETH_Success(uint256 _percentage) external {
        uint256 percentage = bound(_percentage, 1, aaveActions.PERCENTAGE_FACTOR());
        ...
        IAction.PluginExecution[] memory executions = aaveActions.borrowPercentageOfAvailableETH(WALLET, percentage, 2);
        execute(executions);
    ...
    function supply(uint256 amount, address asset) internal {
        ...
        IAction.PluginExecution[] memory executions = aaveActions.supply(WALLET, asset, amount);
    ...
    function supplyETH(uint256 amount) internal {
        ...
        IAction.PluginExecution[] memory executions = aaveActions.supplyETH(WALLET, amount);
        execute(executions);
    }

    function borrow(uint256 amount, address asset) internal {
        IAction.PluginExecution[] memory executions = aaveActions.borrow(WALLET, asset, amount, 2);
        execute(executions);
    }

    function borrowETH(uint256 amount) internal {
        IAction.PluginExecution[] memory executions = aaveActions.borrowETH(WALLET, amount, 2);
        execute(executions);
    }
